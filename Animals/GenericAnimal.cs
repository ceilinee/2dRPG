...
public class GenericAnimal : AnimalState
{
    ...
    public virtual void CheckDistance(){
      ...
      if(!stop && animalTrait.follow && Vector3.Distance(target.position, transform.position) <= chaseRadius && Vector3.Distance(target.position, transform.position)> attackRadius){
      ...
      else if(!stop && followParent && !animalTrait.follow && Vector3.Distance(parent.position, transform.position) <= chaseRadius + 20 && Vector3.Distance(parent.position, transform.position)> attackRadius){
        if(currentState == AnimalStates.idle || currentState == AnimalStates.walk){
          Vector3 temp = Vector3.MoveTowards(transform.position, parent.position, moveSpeed * Time.deltaTime);
          changeAnim(temp - transform.position);
          myRigidbody.MovePosition(temp);
          ChangeState(AnimalStates.walk);
          anim.SetBool("Follow", true);
